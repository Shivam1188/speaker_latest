...
async def audio_ws(websocket: WebSocket):
    ...
    username = query_params.get("username", [None])[0]
    token = query_params.get("token", [None])[0]
    topic_name = query_params.get("topic", [None])[0]
    class_name = query_params.get("class", [None])[0]
    ...
    config = {
        ...
        'blocklist': [
            ...
            "I'm going to take a picture of the sea", "Kansai International Airport",
    ...
    session_state = {
        ...
        'chunk_results': [],
        'text_output': [],
        'lock': asyncio.Lock()  # Add a lock for thread safety
    }
    ...
    try:
        topic = Topic()  # Initialize your Topic class
                ...
                async with session_state['lock']:
                    ...
                    if current_silence > 0.2:
                        ...
                        if session_state['silence_duration'] >= config['silence_threshold']:
                            await process_buffered_audio(session_state, websocket, username, session_temp_dir, topic, config)
                    else:
                        session_state['silence_duration'] = 0.0
                        session_state['last_speech_time'] = current_time
                        
                        buffer_duration = len(session_state['audio_buffer']) / 1000
                        if buffer_duration >= config['max_chunk_duration']:
                            await process_buffered_audio(session_state, websocket, username, session_temp_dir, topic, config)
        ...
        await finalize_session(session_state, username, session_temp_dir, topic)

async def scraping(topic: str) -> str:
    """Async version of scraping function"""
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(api_endpoint) as response:
                if response.status == 200:
                    ...
                    logging.warning(f"[SCRAPING] Failed with status {response.status}")
                    return ""
    except Exception as e:
...
async def process_buffered_audio(session_state, websocket, username, temp_dir, topic, config):
...
async def process_user_utterance(text, emotion, fluency, pronunciation, 
                               ...
                               username, session_temp_dir, topic):
...
5. And use {text} for relevant context.
...
""")
        ...
        ai_response = qa_chain.invoke({"input": text})
